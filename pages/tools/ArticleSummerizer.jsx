...
function ArticleSummerizer() {
  const [loading, setLoading] = useState(false);
  const [article, setArticle] = useState("");
  const [language, setLanguage] = useState("English");
  const [response, setResponse] = useState("");

  const prompt = ` summerize the following article: ${article}  in ${language} language`;
      ...
      <main className="mb-20 flex flex-row gap-8 w-full flex-wrap  justify-center text-center px-4 mt-8 sm:mt-20">
        <div className="shadow-xl flex flex-col bg-white rounded-md p-4 w-full sm:w-[500px] ">
          ...
          <div className="w-full  flex-1">
            <p className="font-semibold text-left mt-4 mb-2">Article </p>
            <textarea
              value={article}
              ...
              placeholder={"Enter URL or Paste Article"}
            />
...
export default ArticleSummerizer;
